...
//#define CHANNEL A13
//#define CHANNEL 15
const uint16_t samples = 16384; //This value MUST ALWAYS be a power of 2  16384
...
//
//    uint16_t min_idx = (uint16_t)(((4000) * ((double)samples))/samplingFrequency);
//    uint16_t max_idx = (uint16_t)(((10000) * ((double)samples))/samplingFrequency); 
//
...
////  Serial.println(freq_band - f_rx);
  ...
  if (f_rx < 8000) {
//    if (f_rx<2000){
//      uint16_t floor_idx = (uint16_t)(((f_rx + 15) * ((double)samples))/samplingFrequency);
//      uint16_t ceil_idx = (uint16_t)(((f_rx - 15) * ((double)samples))/samplingFrequency);
//
//      
//    
//      
...
//  Serial.println(freq_band - f_rx);
  ...
  float spin_rate = ((freq_band-f_rx) *60.0)/2;
//  Serial.print("\nSpin Rate: ");
